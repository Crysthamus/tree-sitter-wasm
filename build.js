...
/**
 * Generates the tree-sitter parser unconditionally or falls back to grammar.json.
 */
async function generateParser(grammarDir) {
	console.log(`Generating parser for ${grammarDir}...`);
	try {
		let generateCmd = `"${TS_BIN}" generate`;

		try {
			await fs.access(path.join(grammarDir, "grammar.js"));
		} catch (err) {
			if (err.code === "ENOENT") {
				generateCmd = `"${TS_BIN}" generate src/grammar.json`;
			} else {
				throw err;
			}
		}

		await execAsync(generateCmd, { cwd: grammarDir });
	} catch (error) {
		throw new Error(
...
/**
 * Routes a dependency to source compilation with caching.
 */
async function processDependency(dep, requestedVersion, cache) {
	...
	const builtResults = await processSourceGrammar(
		grammarDirs,
		depPath,
		baseCleanName,
	);
